 ...
 */
public class ExamplePieceTests5
{
  /**
  ...
  private static Piece makeShape(int row, int col)
  {
    ...
    return new DiagonalPiece(new Position(row, col), icons);
...
//    System.out.println(Arrays.toString(shape.getCellsAbsolute()));
    ...
    // after moving the copy, original should be unmodified
    // expected [R(0, 0), G(0, 1), B(1, 1), M(2, 1)]
    //          [R(1, 0), G(1, 1), B(2, 1), M(3, 1)]
    copy.shiftDown();
    System.out.println(Arrays.toString(shape.getCellsAbsolute()));
    System.out.println(Arrays.toString(copy.getCellsAbsolute()));
    System.out.println();
    
    // after transforming the copy, original should be unmodified
    // expected [R(0, 0), G(0, 1), B(1, 1), M(2, 1)]
    //          [R(1, 2), G(1, 1), B(2, 1), M(3, 1)]
    copy.transform();
    System.out.println(Arrays.toString(shape.getCellsAbsolute()));
    System.out.println(Arrays.toString(copy.getCellsAbsolute()));
    System.out.println();
    
    // cycle followed by transform should work
    // expected [M(0, 2), R(0, 1), G(1, 1), B(2, 1)]
    shape = makeShape(0, 0);
    shape.cycle();
    shape.transform();
    System.out.println(Arrays.toString(shape.getCellsAbsolute()));

    // the copy should still be the same
    // expected [R(1, 2), G(1, 1), B(2, 1), M(3, 1)]
    System.out.println(Arrays.toString(copy.getCellsAbsolute()));
  }
}
